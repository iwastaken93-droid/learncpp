title: "2.11 — Header files – Learn C++"
...
[*Alex*][1] June 3, 2007, 9:29 am PDT February 27, 2025

In lesson [2.8 -- Programs with multiple code files][2], we discussed how programs can be split across multiple files. We also discussed how forward declarations are used to allow the code in one file to access something defined in another file.
...
1.  A *header guard*, which we'll discuss in more detail in the next lesson ([2.12 -- Header guards][3]).
...
Adding a header file to a project works analogously to adding a source file (covered in lesson [2.8 -- Programs with multiple code files][2]).
...
We covered the one-definition rule (ODR) in lesson [2.7 -- Forward declarations and definitions][5].
...
| Header type           | Naming convention | Example    | Identifiers placed in namespace     |
|:----------------------|:------------------|:-----------|:------------------------------------|
| C++ specific (new)    |                   | iostream   | `std` namespace                     |
| C compatibility (new) |                   | cstddef    | `std` namespace (required) <br/>global namespace (optional) |
| C++ specific (old)    |                   | iostream.h | Global namespace                    |
| C compatibility (old) |                   | stddef.h   | Global namespace (required) <br/> `std` namespace (optional) |
...
2.10Introduction to the preprocessor
  ...
  [4]: https://www.learncpp.com/images/CppTutorial/Section1/IncludeHeader.png
