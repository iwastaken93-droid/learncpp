title: "12.7 — Introduction to pointers – Learn C++"
...
# 12.7 â€" Introduction to pointers

[*Alex*][1] July 10, 2007, 6:52 pm PDT February 7, 2025
...
If you're rusty or not familiar with lvalue references, now would be a good time to review them. We cover lvalue references in lessons [12.3 -- Lvalue references][2], [12.4 -- Lvalue references to const][3], and [12.5 -- Pass by lvalue reference][4].
...
In the above example, we use the address-of operator (&) to retrieve the address assigned to variable `x` and print that address to the console. Memory addresses are typically printed as hexadecimal values (we covered hex in lesson
...
In modern C++, the pointers we are talking about here are sometimes called "raw pointers� or "dumb pointers�, to help differentiate them from "smart pointers� that were introduced into the language more recently. We cover smart pointers in [chapter 22][5].
...
12.6Pass by const lvalue reference
  ...
  [4]: https://www.learncpp.com/cpp-tutorial/pass-by-lvalue-reference/
  [5]: https://www.learncpp.com#Chapter22
  [6]: https://www.learncpp.com/images/CppTutorial/Section6/6-Pointer.png
  [7]: javascript:void(0)
