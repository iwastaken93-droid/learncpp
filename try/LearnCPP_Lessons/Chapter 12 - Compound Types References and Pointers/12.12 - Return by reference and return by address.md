title: "12.12 — Return by reference and return by address – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/return-by-reference-and-return-by-address/
...
# 12.12 â€" Return by reference and return by address

[*Alex*][1] February 25, 2008, 9:04 pm PST January 3, 2025
...
Returning a const reference to a *const* global variable is also sometimes done as a way to encapsulate access to a global variable. We discuss this in lesson [7.8 -- Why (non-const) global variables are evil][2]. When used intentionally and carefully, this is also okay.
...
We discuss a similar case in lesson [14.6 -- Access functions][3].
...
If you need the ability to return "no object� or a value (rather than an object) [12.15 -- std::optional][4] describes a good alternative.

Related content

See [5.9 -- std::string_view (part 2)][5] for a quick guide on when to return `std::string_view` vs `const std::string&` .
