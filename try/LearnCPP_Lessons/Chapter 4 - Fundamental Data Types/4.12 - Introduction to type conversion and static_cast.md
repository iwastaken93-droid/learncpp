title: "4.12 — Introduction to type conversion and static_cast – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/introduction-to-type-conversion-and-static_cast/
...
# 4.12 â€" Introduction to type conversion and static_cast

[*Alex*][1] October 21, 2021, 2:27 pm PDT March 3, 2025
...
You'll need to disable "treat warnings as errors� temporarily if you want to compile this example. See lesson [0.11 -- Configuring your compiler: Warning and error levels][2] for more information about this setting.
...
Implicit type conversion is a meaty topic. We dig into this topic in more depth in future lessons, starting with lesson [10.1 -- Implicit type conversion][3].
...
C++ supports other types of casts. We talk more about the different types of casts in future lesson [10.6 -- Explicit type conversion (casting) and static_cast][4].

Using static_cast to convert char to int

In the lesson on chars [4.11 -- Chars][5], we saw that printing a char value using `std::cout` results in the value being printed as a char:
...
- If the destination type is unsigned, the value will be modulo wrapped. We cover modulo wrapping in lesson [4.5 -- Unsigned integers, and why to avoid them][6].
...
As noted in lesson [4.6 -- Fixed-width integers and size_t][7], most compilers define and treat `std::int8_t` and `std::uint8_t` (and the corresponding fast and least fixed-width types) identically to types `signed char` and `unsigned char` respectively. Now that we've covered what chars are, we can demonstrate where this can be problematic:
...
4.11Chars
  ...
  [2]: https://www.learncpp.com/cpp-tutorial/configuring-your-compiler-warning-and-error-levels/
  [3]: https://www.learncpp.com/cpp-tutorial/implicit-type-conversion/
  [4]: https://www.learncpp.com/cpp-tutorial/explicit-type-conversion-casting-and-static-cast/
  [5]: https://www.learncpp.com/cpp-tutorial/chars/
  [6]: https://www.learncpp.com/cpp-tutorial/unsigned-integers-and-why-to-avoid-them/
  [7]: https://www.learncpp.com/cpp-tutorial/fixed-width-integers-and-size-t/
  [8]: javascript:void(0)
  [9]: https://www.learncpp.com/cpp-tutorial/chapter-4-summary-and-quiz/
