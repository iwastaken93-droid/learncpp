title: "4.7 — Introduction to scientific notation – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/introduction-to-scientific-notation/
...
# 4.7 â€" Introduction to scientific notation

[*Alex*][1] April 23, 2019, 12:55 pm PDT January 29, 2025
...
Numbers in scientific notation take the following form: *significand* x 10^*exponent*^. For example, in the scientific notation `1.2 x 10â?´`, `1.2` is the significand and `4` is the exponent. Since 10â?´ evaluates to 10,000, 1.2 x 10â?´ evaluates to 12,000.
...
In scientific notation, this would be written as `5.9722 x 10 ²â?´ kg`, which is much easier to read. Scientific notation has the added benefit of making it easier to compare the magnitude of two extremely large or small numbers simply by comparing the exponent.

Because it can be hard to type or display exponents in C++, we use the letter 'e' (or sometimes 'E') to represent the "times 10 to the power ofâ€? part of the equation. For example, `1.2 x 10â?´` would be written as `1.2e4`, and `5.9722 x 10 ²â?´` would be written as `5.9722e24`.

For numbers smaller than 1, the exponent can be negative. The number `5e-2` is equivalent to `5 * 10â?» ²`, which is `5 / 10 ²`, or `0.05`. The mass of an electron is `9.1093837e-31 kg`.

For Linux users

If you're using arch linux and the superscript of `5 * 10â?» ²` is missing the negative sign, you may need to install a font that can display these characters. See [this reddit thread for more info][2].
...
3.45000e4 (6 significant digits). We don't trim the trailing zeros here because the number *does* have a decimal point. Even though the decimal point doesn't affect the value of the number, it affects the precision, so it needs to be included in the significand.
...
4.6Fixed-width integers and size_t
  ...
  [2]: https://www.reddit.com/r/archlinux/comments/1af46vq/some_unicode_characters_not_rendering_properly/
  [3]: javascript:void(0)
  [4]: https://www.learncpp.com/cpp-tutorial/floating-point-numbers/
  [5]: https://www.learncpp.com/
  [6]: https://www.learncpp.com/cpp-tutorial/fixed-width-integers-and-size-t/
