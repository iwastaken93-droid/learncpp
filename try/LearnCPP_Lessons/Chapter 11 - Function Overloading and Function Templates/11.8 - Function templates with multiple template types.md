title: "11.8 — Function templates with multiple template types – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/function-templates-with-multiple-template-types/
...
# 11.8 â€" Function templates with multiple template types

[*Alex*][1] June 17, 2021, 5:46 pm PDT December 18, 2024

In lesson [11.6 -- Function templates][2], we wrote a function template to calculate the maximum of two values:
...
If we need a function that can be forward declared, we have to be explicit about the return type. Since our return type needs to be the common type of `T` and `U`, we can use `std::common_type_t` (discussed in lesson [10.5 -- Arithmetic conversions][4]) to fetch the common type of `T` and `U` to use as our explicit return type:
...
11.9Non-type template parameters

Back to table of contents

Previous lesson

11.7Function template instantiation
