title: "7.13 — Using declarations and using directives – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/using-declarations-and-using-directives/
...
# 7.13 â€" Using declarations and using directives

[*Alex*][1] November 9, 2016, 6:23 pm PST March 5, 2025
...
For technical reasons, using-directives do not actually introduce new meanings for names into the current scope -- instead they introduce new meanings for names into an outer scope (more details about which outer scope is picked can be found [here][2]).
...
This example uses a concept we haven't covered yet called "function overloading� (we cover this in lesson [11.1 -- Introduction to function overloading][3]). All you need to know for this example is that two functions in the same scope can have the same name so long as the parameters of each are distinct. Since `int` and `double` are distinct types, there is no issue having both `Foo::print(int)` and `Foo::print(double)` living side-by-side.
...
The `using` keyword is also used to define type aliases, which are unrelated to using-statements. We cover type aliases in lesson [10.7 -- Typedefs and type aliases][4].

Next lesson

7.14Unnamed and inline namespaces

Back to table of contents

Previous lesson

7.12Scope, duration, and linkage summary

  [1]: https://www.learncpp.com/author/Alex/
  [2]: https://quuxplusone.github.io/blog/2020/12/21/using-directive/
  ...
  [5]: https://www.learncpp.com/cpp-tutorial/unnamed-and-inline-namespaces/
