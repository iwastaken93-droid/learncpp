title: "7.2 — User-defined namespaces and the scope resolution operator – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/user-defined-namespaces-and-the-scope-resolution-operator/
...
[*Alex*][1] August 17, 2007, 6:09 pm PDT June 28, 2024

In lesson [2.9 -- Naming collisions and an introduction to namespaces][2], we introduced the concept of `naming collisions` and `namespaces`. As a reminder, a naming collision occurs when two identical identifiers are introduced into the same scope, and the compiler can't disambiguate which one to use. When this happens, compiler or linker will produce an error because they do not have enough information to resolve the ambiguity.
...
In lesson [2.11 -- Header files][3], we discussed how we can use header files to propagate forward declarations. For identifiers inside a namespace, those forward declarations also need to be inside the same namespace:
...
C++ provides other useful namespace functionality. We cover unnamed namespaces and inline namespaces later in this chapter, in lesson [7.14 -- Unnamed and inline namespaces][4].
...
7.1Compound statements (blocks)
  ...
  [4]: https://www.learncpp.com/cpp-tutorial/unnamed-and-inline-namespaces/
  [5]: https://www.learncpp.com/cpp-tutorial/local-variables/
  [6]: https://www.learncpp.com/
  [7]: https://www.learncpp.com/cpp-tutorial/compound-statements-blocks/
