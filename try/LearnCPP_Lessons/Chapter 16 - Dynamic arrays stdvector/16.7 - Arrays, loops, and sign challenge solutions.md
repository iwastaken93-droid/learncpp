title: "16.7 — Arrays, loops, and sign challenge solutions – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/arrays-loops-and-sign-challenge-solutions/
...
# 16.7 â€" Arrays, loops, and sign challenge solutions

[*Alex*][1] September 11, 2023, 2:34 pm PDT October 19, 2024

In lesson [4.5 -- Unsigned integers, and why to avoid them][2], we noted how we generally prefer to use signed values to hold quantities, because unsigned values can act in surprising ways. However, in lesson
...
1.  In cases where you can derive the type of your loop variable from the initializer, you can use `auto` to have the compiler deduce the type:
...
This uses operator overloading (which we briefly introduced in lesson [13.5 -- Introduction to overloading the I/O operators][3]) to implement `operator[]`. You don't need to know how `SignedArrayView` is implemented in order to use it.
...
SignedArrayView.h:
    ...
    public:
        ...
        constexpr auto& operato { return m_array[static_cast<typename T::size_type>(index)]; }
        constexpr const auto& operato const { return m_array[static_cast<typename T::size_type>(index)]; }
...
We cover ranged-for loops in the next lesson ([16.8 -- Range-based for loops (for-each)][4]).  
We cover iterators in upcoming lesson [18.2 -- Introduction to iterators][5].
...
16.8Range-based for loops (for-each)

Back to table of contents

Previous lesson

16.6Arrays and loops
  ...
  [4]: https://www.learncpp.com/cpp-tutorial/range-based-for-loops-for-each/
  [5]: https://www.learncpp.com/cpp-tutorial/introduction-to-iterators/
  [6]: https://www.learncpp.com/
  [7]: https://www.learncpp.com/cpp-tutorial/arrays-and-loops/
