title: "16.3 — std::vector and the unsigned length and subscript problem – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/stdvector-and-the-unsigned-length-and-subscript-problem/
date: 2026-04-01 03:26
tags: 
description: 
...
# 16.3 â€" std::vector and the unsigned length and subscript problem

[*Alex*][1] September 11, 2023, 2:28 pm PDT November 11, 2024

In the prior lesson [16.2 -- Introduction to std::vector and list constructors][2], we introduced `operator[]`, which can be used to subscript an array to access an element.
...
As Bjarne Stroustrup [recalls][3], when the container classes in the C++ standard library was being designed (circa 1997), the designers had to choose whether to make the length (and array subscripts) signed or unsigned. They chose to make them unsigned.
...
regarding sign conversions (integral conversions from signed to unsigned or vice-versa) because we'll be talking about these a lot in this chapter.
...
You'll typically see `size_type` appear in documentation and in compiler warnings/error messages. For example, this [documentation for the `size()` member function][6] of `std::vector` indicates that `size()` returns a value of `size_type`.

Related content

We cover nested typedefs in lesson [15.3 -- Nested types (member types)][7].
...
All of the standard library containers except `std::array` use [`std::allocator`][8] to allocate memory. For these containers, `T::size_type` is derived from the `size_type` of the allocator used. Since `std::allocator` can allocate up to `std::size_t` bytes of memory, `std::allocator<T>::size_type` is defined as `std::size_t`. Therefore, `T::size_type` defaults to `std::size_t`.
...
We discuss C-style array decay in lesson [17.8 -- C-style array decay][9].
...
When the `at()` member function encounters an out-of-bounds index, it actually throws an exception of type `std::out_of_range`. If the exception is not handled, the program will be terminated. We cover exceptions and how to handle them in [chapter 27][10].
...
However, as per our best practices ([4.5 -- Unsigned integers, and why to avoid them][4]), we generally want to avoid using unsigned types to hold quantities.
...
16.4Passing std::vector

Back to table of contents

Previous lesson

16.2Introduction to std::vector and list constructors

  [1]: https://www.learncpp.com/author/Alex/
  [2]: https://www.learncpp.com/cpp-tutorial/introduction-to-stdvector-and-list-constructors/
  [3]: https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2019/p1428r0.pdf
  [4]: https://www.learncpp.com/cpp-tutorial/unsigned-integers-and-why-to-avoid-them/
  [5]: https://www.learncpp.com/cpp-tutorial/typedefs-and-type-aliases/
  [6]: https://en.cppreference.com/w/cpp/container/vector/size
  [7]: https://www.learncpp.com/cpp-tutorial/nested-types-member-types/
  [8]: https://en.cppreference.com/w/cpp/memory/allocator
  [9]: https://www.learncpp.com/cpp-tutorial/c-style-array-decay/
  [10]: https://www.learncpp.com#Chapter27
  [11]: https://www.learncpp.com/cpp-tutorial/introduction-to-c-style-arrays/
  [12]: https://www.learncpp.com/cpp-tutorial/arrays-loops-and-sign-challenge-solutions/
  [13]: javascript:void(0)
  [14]: https://www.learncpp.com/cpp-tutorial/passing-stdvector/
  [15]: https://www.learncpp.com/
