title: "F.1 — Constexpr functions – Learn C++"
...
[*Alex*][1] March 28, 2022, 9:12 am PDT February 18, 2025

In lesson [5.6 -- Constexpr variables][2], we introduced the `constexpr` keyword, which we used to create compile-time (symbolic) constants. We also introduced constant expressions, which are expressions that can be evaluated at compile-time rather than runtime.
...
In lesson [5.5 -- Constant expressions][3], we noted that in contexts that require a constant expression (such as the initialization of a constexpr variable), a constant expression is required to evaluate at compile-time. If a required constant expression contains a constexpr function call, that constexpr function call must evaluate at compile-time.
...
There are some other lesser encountered criteria as well. These can be found [here][4].
...
Now would be a great time to review the benefits that compile-time programming techniques can provide: [5.5 -- Constant expressions][5].
...
11.xChapter 11 summary and quiz
  ...
  [2]: https://www.learncpp.com/cpp-tutorial/constexpr-variables/
  [3]: https://www.learncpp.com/cpp-tutorial/constant-expressions/
  [4]: https://en.cppreference.com/w/cpp/language/constexpr
  [5]: https://www.learncpp.com/cpp-tutorial/constant-expressions/#compiletimebenefits
  [6]: https://www.learncpp.com/cpp-tutorial/constexpr-functions-part-2/
  [7]: https://www.learncpp.com/
  [8]: https://www.learncpp.com/cpp-tutorial/chapter-11-summary-and-quiz/
