title: "14.10 — Constructor member initializer lists – Learn C++"
...
[*Alex*][1] November 13, 2007, 10:03 am PST February 5, 2025

This lesson continues our introduction of constructors from lesson [14.9 -- Introduction to constructors][2].
...
We discussed class invariants in lesson [14.2 -- Introduction to classes][3].
...
In lesson [9.4 -- Detecting and handling errors][4], we introduced the topic of error handling, and discussed some options for handling cases where a function cannot proceed due to an error occurring. Since constructors are functions, they are susceptible to the same issues.
...
Throwing an exception is usually the best thing to do when a constructor fails (and cannot recover). We discuss this further in lessons [27.5 -- Exceptions, classes, and inheritance][5] and [27.7 -- Function try blocks][6].
...
In the following example, our `createFraction()` function returns a `std::optional<Fraction>` that optionally contains a valid `Fraction`. If it does, then we can use that Fraction. If not, then the caller can detect that and deal with it. We cover `std::optional` in lesson [12.15 -- std::optional][7] and friend functions in lesson [15.8 -- Friend non-member functions][8].
...
The rationale for this is given in lesson [14.8 -- The benefits of data hiding (encapsulation)][10].
...
We discuss this case in more detail (and show an example) in lesson [13.11 -- Struct miscellany][11].

Next lesson

14.11Default constructors and default arguments
