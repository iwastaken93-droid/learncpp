title: "20.x — Chapter 20 summary and quiz – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/chapter-20-summary-and-quiz/
...
# 20.x â€" Chapter 20 summary and quiz

[*Alex*][1] December 4, 2015, 7:31 pm PST February 8, 2025
...
In C++20, calculating the midpoint can be done via calling [`std::midpoint`][3]. Prior to C++20, you'll need to calculate the midpoint yourself -- in this case you can use `min + ((max - min) / 2)` to avoid overflow (when min and max are both positive).
...
[`std::binary_search`][4] returns true if a value exists in a sorted list.  
[`std::equal_range`][5] returns the iterators to the first and last element with a given value.

Don't use these functions to solve the quiz, but use them in the future if you need a binary search.

Next lesson

21.1Introduction to operator overloading
