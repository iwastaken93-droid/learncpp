title: "5.4 — The as-if rule and compile-time optimization – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/the-as-if-rule-and-compile-time-optimization/
...
# 5.4 â€" The as-if rule and compile-time optimization

[*Alex*][1] October 22, 2024, 12:48 pm PDT January 1, 2025
...
See [0.9 -- Configuring your compiler: Build configurations][2] for information on how to enable optimization.
...
There is one notable exception to the as-if rule: unnecessary calls to a copy (or move) constructor can be elided (omitted) even if those constructors have observable behavior. We cover this topic in lesson [14.15 -- Class initialization and copy elision][3].
...
[Wikipedia][4] has list of specific techniques that compilers use.
...
Compile-time debugging is an underdeveloped area. As of C++23, there are a number of papers under consideration for future language standards (such as [this one][5]) that (if approved) will add capabilities to the language that will help.
...
- Some compile-time constants (e.g. `const double d { 1.2 };`) cannot be used in compile-time features (as defined by the language standard). We'll discuss this more in lesson [5.5 -- Constant expressions][6].
...
5.5Constant expressions

Back to table of contents

Previous lesson

5.3Numeral systems (decimal, binary, hexadecimal, and octal)
  ...
  [4]: https://en.wikipedia.org/wiki/Optimizing_compiler#Specific_techniques
  [5]: https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2023/p2758r1.html
  [6]: https://www.learncpp.com/cpp-tutorial/constant-expressions/
