title: "5.9 — std::string_view (part 2) – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/stdstring_view-part-2/
...
# 5.9 â€" std::string_view (part 2)

[*Alex*][1] June 16, 2022, 2:19 pm PDT January 2, 2025

In prior lessons, we introduced two string types: `std::string` ([5.7 -- Introduction to std::string][2]) and `std::string_view` ([5.8 -- Introduction to std::string_view][3]).
...
Prefer `std::string_view` in most cases. We cover this topic further in lesson [12.6 -- Pass by const lvalue reference][4].
...
We cover what a null-terminated string is in lesson [5.2 -- Literals][5].
...
Also see [12.6 -- Pass by const lvalue reference][4].
...
- You are using a `std::string` as an out-parameter (see [12.13 -- In and out parameters][6]).
...
See lesson [12.12 -- Return by reference and return by address][7] for more information on returning reference types.
...
5.8Introduction to std::string_view
  ...
  [5]: https://www.learncpp.com/cpp-tutorial/literals/
  ...
  [8]: https://www.learncpp.com/cpp-tutorial/chapter-5-summary-and-quiz/
