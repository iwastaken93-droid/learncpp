title: "5.5 — Constant expressions – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/constant-expressions/
...
# 5.5 â€" Constant expressions

[*Alex*][1] June 16, 2022, 2:19 pm PDT February 18, 2025

In lesson [1.10 -- Introduction to expressions][2], we introduced expressions. By default, expressions evaluate at runtime. And in some cases, they must do so:
...
In prior lesson [5.4 -- The as-if rule and compile-time optimization][3], we discussed the as-if rule, and how the compiler can optimize programs by shifting work from runtime to compile-time. Under the as-if rule, the compiler may choose whether to evaluate certain expressions at runtime or compile-time:
...
We'll discuss constexpr variables in the next lesson ([5.6 -- Constexpr variables][4]), when we cover constexpr variables.

For advanced readers

A few common cases where a compile-time evaluatable expression is required:

- The initializer of a constexpr variable ([5.6 -- Constexpr variables][4]).
- A non-type template argument ([11.9 -- Non-type template parameters][5]).
- The defined length of a `std::array` ([17.1 -- Introduction to std::array][6]) or a C-style array ([17.7 -- Introduction to C-style arrays][7]).
...
- Constexpr variables (discussed in upcoming lesson [5.6 -- Constexpr variables][4]).
- Constexpr functions (discussed in upcoming lesson [F.1 -- Constexpr functions][8]).
- Templates (introduced in lesson [11.6 -- Function templates][9]).
- static_assert (discussed in lesson [9.6 -- Assert and static_assert][10]).
...
- Constexpr variables (discussed in upcoming lesson [5.6 -- Constexpr variables][4]).
- Constexpr function calls with constant expression arguments (see [F.1 -- Constexpr functions][8]).

For advanced readers

Constant expressions can also contain:

- Non-type template parameters (see [11.9 -- Non-type template parameters][5]).
- Enumerators (see [13.2 -- Unscoped enumerations][12]).
- Type traits (see the [cppreference page for type traits][13]).
- Constexpr lambda expressions (see [20.6 -- Introduction to lambdas (anonymous functions)][14]).
...
- Non-const variables.
- Const non-integral variables, even when they have a constant expression initializer (e.g. `const double d { 1.2 };`). To use such variables in a constant expression, define them as constexpr variables instead (see lesson [5.6 -- Constexpr variables][4]).
...
For the precise definition of a constant expression, see the [cppreference page for constant expression][15]. Note that a constant expression is defined by what kind of expression it is not. That means we're left to infer what it is. Good luck with that!
...
5.6Constexpr variables

Back to table of contents

Previous lesson

5.4The as-if rule and compile-time optimization

  [1]: https://www.learncpp.com/author/Alex/
  [2]: https://www.learncpp.com/cpp-tutorial/introduction-to-expressions/
  [3]: https://www.learncpp.com/cpp-tutorial/the-as-if-rule-and-compile-time-optimization/
  [4]: https://www.learncpp.com/cpp-tutorial/constexpr-variables/
  [5]: https://www.learncpp.com/cpp-tutorial/non-type-template-parameters/
  [6]: https://www.learncpp.com/cpp-tutorial/introduction-to-stdarray/
  [7]: https://www.learncpp.com/cpp-tutorial/introduction-to-c-style-arrays/
  [8]: https://www.learncpp.com/cpp-tutorial/constexpr-functions/
  [9]: https://www.learncpp.com/cpp-tutorial/function-templates/
  [10]: https://www.learncpp.com/cpp-tutorial/assert-and-static_assert/
  [11]: https://www.learncpp.com/cpp-tutorial/introduction-to-the-preprocessor/
  [12]: https://www.learncpp.com/cpp-tutorial/unscoped-enumerations/
  [13]: https://en.cppreference.com/w/cpp/header/type_traits
  [14]: https://www.learncpp.com/cpp-tutorial/introduction-to-lambdas-anonymous-functions/
  [15]: https://en.cppreference.com/w/cpp/language/constant_expression
  [16]: javascript:void(0)
  [17]: https://www.learncpp.com/
