title: "19.4 — Pointers to pointers and dynamic multidimensional arrays – Learn C++"
...
# 19.4 â€" Pointers to pointers and dynamic multidimensional arrays

[*Alex*][1] September 14, 2015, 3:44 pm PDT September 9, 2024
...
Another common use for pointers to pointers is to facilitate dynamically allocated multidimensional arrays (see [17.12 -- Multidimensional C-style Arrays][2] for a review of multidimensional arrays).
...
However, if we want a function to be able to modify what a pointer argument points to, this is generally better done using a reference to a pointer instead. This is covered in lesson [12.11 -- Pass by address (part 2)][3].
...
We recommend avoiding using pointers to pointers unless no other options are available, because they're complicated to use and potentially dangerous. It's easy enough to dereference a null or dangling pointer with normal pointers â€" it's doubly easy with a pointer to a pointer since you have to do a double-dereference to get to the underlying value!
