title: "19.2 — Dynamically allocating arrays – Learn C++"
...
# 19.2 â€" Dynamically allocating arrays

[*Alex*][1] August 18, 2015, 1:52 pm PDT November 20, 2023
...
In lesson [17.8 -- C-style array decay][2], you learned that a fixed array holds the memory address of the first array element. You also learned that a fixed array can decay into a pointer that points to the first element of the array. In this decayed form, the length of the fixed array is not available (and therefore neither is the size of the array via sizeof()), but otherwise there is little difference.
...
You can use `std::getline()` to read in names that contain spaces (see lesson [5.7 -- Introduction to std::string][5]).
...
19.1Dynamic memory allocation with new and delete
