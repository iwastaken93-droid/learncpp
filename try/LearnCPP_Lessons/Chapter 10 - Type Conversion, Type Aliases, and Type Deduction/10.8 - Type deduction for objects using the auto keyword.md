title: "10.8 — Type deduction for objects using the auto keyword – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/type-deduction-for-objects-using-the-auto-keyword/
...
# 10.8 â€" Type deduction for objects using the auto keyword

[*Alex*][1] July 30, 2015, 1:02 pm PDT September 25, 2024
...
We discuss how literal types are determined in lesson [5.2 -- Literals][2].
...
The type deduction rules for pointers and references are a bit more complex. We discuss these in [12.14 -- Type deduction with pointers, references, and const][3].
...
If you want the type deduced from a string literal to be `std::string` or `std::string_view`, you'll need to use the `s` or `sv` literal suffixes (introduced in lessons [5.7 -- Introduction to std::string][4] and [5.8 -- Introduction to std::string_view][5]):
...
10.9Type deduction for functions
