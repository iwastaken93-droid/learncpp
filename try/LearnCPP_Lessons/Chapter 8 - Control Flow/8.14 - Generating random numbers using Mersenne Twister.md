title: "8.14 — Generating random numbers using Mersenne Twister – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/generating-random-numbers-using-mersenne-twister/
...
# 8.14 â€" Generating random numbers using Mersenne Twister

[*Alex*][1] December 7, 2008, 2:43 pm PST February 2, 2025

In the previous lesson [8.13 -- Introduction to random number generation][2], we introduced the concept of random number generation, and discussed how PRNG algorithms are typically used to simulate randomness in programs.
...
In lesson [5.7 -- Introduction to std::string][3], we showed an example where we called the function `name.length()`, which invoked the `length()` function on `std::string` variable `name`.
...
In the prior lesson ([8.13 -- Introduction to random number generation][2]), we covered that each number in a PRNG sequence is in a deterministic way. And that the state of the PRNG is initialized from the seed value. Thus, given any starting seed number, PRNGs will always generate the same sequence of numbers from that seed as a result.
...
One potential problem with `std::random_device`: it isn't required to be non-deterministic, meaning it *could*, on some systems, produce the same sequence every time the program is run, which is exactly what we're trying to avoid. There was a [bug in MinGW][4] (fixed in GCC 9.2) that would do exactly this, making `std::random_device` useless.
...
This content was moved to [8.15 -- Global random numbers (Random.h)][7].
...
8.15Global random numbers (Random.h)

Back to table of contents

Previous lesson

8.13Introduction to random number generation

  [1]: https://www.learncpp.com/author/Alex/
  [2]: https://www.learncpp.com/cpp-tutorial/introduction-to-random-number-generation/
  [3]: https://www.learncpp.com/cpp-tutorial/introduction-to-stdstring/
  [4]: https://gcc.gnu.org/bugzilla/show_bug.cgi?id=85494
  [5]: https://www.pcg-random.org/posts/ease-of-use-without-loss-of-power.html
  [6]: https://gist.github.com/imneme/540829265469e673d045
