title: "13.4 — Converting an enumeration to and from a string – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/converting-an-enumeration-to-and-from-a-string/
...
# 13.4 â€" Converting an enumeration to and from a string

[*Alex*][1] March 25, 2024, 2:51 pm PDT February 13, 2025

In the prior lesson ([13.3 -- Unscoped enumerator integral conversions][2]), we showed an example like this:
...
In lesson [8.5 -- Switch statement basics][3], we noted that a switch statement can switch on either an integral value or an enumerated value. In the following example, we use a switch statement to select an enumerator and return the appropriate color string literal for that enumerator:
...
Constexpr functions are covered in lesson [F.1 -- Constexpr functions][4].

While this lets us get the name of an enumerator, if we want to print that name to the console, having to do `std::cout << getColorName(shirt)` isn't quite as nice as `std::cout << shirt`. We'll teach `std::cout` how to print an enumeration in upcoming lesson [13.5 -- Introduction to overloading the I/O operators][5].

The second way to solve the program of mapping enumerators to strings is to use an array. We cover this in lesson [17.6 -- std::array and enumerations][6].
...
We cover `std::optional` in lesson [12.15 -- std::optional][7].
...
We cover lambdas in lesson [20.6 -- Introduction to lambdas (anonymous functions)][8].

Similar to the output case, it would be better if we could just `std::cin >> pet`. We'll cover this in upcoming lesson [13.5 -- Introduction to overloading the I/O operators][5].
...
13.3Unscoped enumerator integral conversions
  ...
  [2]: https://www.learncpp.com/cpp-tutorial/unscoped-enumerator-integral-conversions/
  [3]: https://www.learncpp.com/cpp-tutorial/switch-statement-basics/
  [4]: https://www.learncpp.com/cpp-tutorial/constexpr-functions/
  [5]: https://www.learncpp.com/cpp-tutorial/introduction-to-overloading-the-i-o-operators/
  [6]: https://www.learncpp.com/cpp-tutorial/stdarray-and-enumerations/
  [7]: https://www.learncpp.com/cpp-tutorial/stdoptional/
  [8]: https://www.learncpp.com/cpp-tutorial/introduction-to-lambdas-anonymous-functions/
