...
# 13.2 â€" Unscoped enumerations

[*Alex*][1] June 19, 2007, 8:24 pm PDT October 28, 2024

C++ contains many useful fundamental and compound data types (which we introduced in lessons [4.1 -- Introduction to fundamental data types][2] and [12.1 -- Introduction to compound data types][3]). But these types aren't always sufficient for the kinds of things we want to do.
...
But this isn't at all intuitive, and we've already discussed why magic numbers are bad ([5.2 -- Literals][4]). We can get rid of the magic numbers by using symbolic constants:
...
Because enumerations are program-defined types [13.1 -- Introduction to program-defined (user-defined) types][5], each enumeration needs to be fully defined before we can use it (a forward declaration is not sufficient).
...
In lesson [O.1 -- Bit flags and bit manipulation via std::bitset][6], we discussed bit flags. Enums can also be used to define a collection of related bit flag positions for use with `std::bitset`:
...
If you're wondering how we can use an enumerator where an integral value is expected, unscoped enumerators will implicitly convert to integral values. We will explore this further in the next lesson ([13.3 -- Unscoped enumerator integral conversions][7]).
...
Classes also provide a scope region, and it's common to put enumerated types related to a class inside the scope region of the class. We discuss this in lesson [15.3 -- Nested types (member types)][8].

A related option is to use a scoped enumeration (which defines its own scope region). We'll discuss scoped enumerations shortly ([13.6 -- Scoped enumerations (enum classes)][9]).
...
13.3Unscoped enumerator integral conversions

Back to table of contents

Previous lesson

13.1Introduction to program-defined (user-defined) types
  ...
  [4]: https://www.learncpp.com/cpp-tutorial/literals/
  [5]: https://www.learncpp.com/cpp-tutorial/introduction-to-program-defined-user-defined-types/
  [6]: https://www.learncpp.com/cpp-tutorial/bit-flags-and-bit-manipulation-via-stdbitset/
  [7]: https://www.learncpp.com/cpp-tutorial/unscoped-enumerator-integral-conversions/
  [8]: https://www.learncpp.com/cpp-tutorial/nested-types-member-types/
