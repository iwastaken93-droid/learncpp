title: "13.y — Using a language reference – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/using-a-language-reference/
date: 2026-04-01 03:24
tags: 
description: 
...
# 13.y â€" Using a language reference

[*nascardriver*][1] January 30, 2020, 9:04 am PST January 2, 2024
...
One such resource is [Stack Overflow][2], where you can ask questions (or better, read the answer to the same question someone before you asked). But sometimes a better first stop is a reference guide. Unlike tutorials, which tend to focus on the most important topics and use informal/common language to make learning easier, reference guides describe C++ precisely using formal terminology. Because of this, reference material tends to be comprehensive, accurate, and… hard to understand.

In this lesson, we'll show how to use [cppreference][3], a popular standard reference that we refer to throughout the lessons, by researching 3 examples.

Overview

Cppreference greets you with an [overview][4] of the core language and libraries:
...
Clicking on "std::string� leads to the page for [`std::basic_string`][8]. There is no page for `std::string`, because `std::string` is a `typedef` for `std::basic_string<char>`, which again can be seen in the `typedef` list:
...
Further down the same page, there's a [list of member functions][10] (the behaviors that a type has). If you want to know what you can do with a type, this list is very convenient. In this list, you'll find a row for `length` (and `size`).

Following the link brings us to the detailed function description of [`length` and `size`][11], which both do the same thing.
...
It's not there, what now? Let's go to [`std::cin`][16] and work our way from there. There's nothing immediately obvious on that page. On the top, we can see the declaration of `std::cin` and `std::wcin`, and it tells us which header we need to include to use `std::cin`:
...
We've used many of these functions already: `operator>>`, `get`, `getline`, `ignore`. Scroll around on that page to get an idea of what else there is in `std::cin`. Then click [`ignore`][21], since that's what we're interested in.

![][22]

On the top of the page there's the function signature and a description of what the function and its two parameters do. The `=` signs after the parameters indicate a **default argument** (we cover this in lesson [11.5 -- Default arguments][23]). If we don't provide an argument for a parameter that has a default value, the default value is used.
...
Why is there a variable definition inside the condition of the `if-statement`? Let's use cppreference to figure out what it does by searching for "cppreference if statement� in our favorite search engine. Doing so leads us to [if statements][25]. At the top, there's a syntax reference.
...
The `init-statement` examples use features and types that we haven't covered yet. You don't have to understand everything you see to understand how the `init-statement` works. Let's skip everything that's too confusing until we find something we can work with:
    ...
    // [20], what's that? Skip.
    if (char buf[20]; std::fgets(buf, 10, stdin)) { m[0] += buf; }
...
The only official source for C++ is [the standard][29] (Free drafts on [github][30]), which is a formal document and not easily usable as a reference.
...
Scrolling to the "Member functions" list, we find [erase](https://en.cppreference.com/w/cpp/string/basic_string/erase). As hinted in the tip above, the first <strong>function overload</strong> is used. It takes 2  `size_type`  (Unsigned integer type) arguments. In our example, 4 and 11. According to the description of (1), it removes " `min(count, size() - index)`  characters starting at  `index` ". Plugging in our arguments, it removes  `min(11, 19 - 4) = 11`  characters starting at index 4.
...
<pre> `str = "I saw a blue car yesterday.";` </pre>
You only need to call one function to <em>replace</em> "red" with "blue".
...
Hint: [ `std::basic_string` ](https://en.cppreference.com/w/cpp/string/basic_string)
[Show Hint](javascript:void(0))
Hint: [Member functions of  `std::basic_string` ](https://en.cppreference.com/w/cpp/string/basic_string#Member_functions)
[Show Hint](javascript:void(0))
Hint: [Modifiers of  `std::basic_string` ](https://en.cppreference.com/w/cpp/string/basic_string#modifiers)
[Show Hint](javascript:void(0))
Hint: [ `std::basic_string::replace` ](https://en.cppreference.com/w/cpp/string/basic_string/replace)
...
14.1Introduction to object-oriented programming
...
13.xChapter 13 summary and quiz
  ...
  [3]: https://cppreference.com
  [4]: https://en.cppreference.com/w/cpp
  [5]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/overview-min.png
  [6]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/string-search-ddg-min.png
  [7]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/strings-lib-min.png
  [8]: https://en.cppreference.com/w/cpp/string/basic_string
  [9]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/typedef-min.png
  [10]: https://en.cppreference.com/w/cpp/string/basic_string#Member_functions
  [11]: https://en.cppreference.com/w/cpp/string/basic_string/size
  [12]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/string-length-overloads-min.png
  [13]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/string-length-example-min.png
  [14]: https://www.learncpp.com/cpp-tutorial/stdcin-and-handling-invalid-input/
  [15]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/ignore-ddg-min.png
  [16]: https://en.cppreference.com/w/cpp/io/cin
  [17]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/cintop-min.png
  [18]: https://en.cppreference.com/w/cpp/io/basic_istream
  [19]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/basic_istream-min.png
  [20]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/members-min.png
  [21]: https://en.cppreference.com/w/cpp/io/basic_istream/ignore
  [22]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/ignore-top-min.png
  [23]: https://www.learncpp.com/cpp-tutorial/default-arguments/
  [24]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/parameters-return-min.png
  [25]: https://en.cppreference.com/w/cpp/language/if
  [26]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/syntax-min.png
  [27]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/explanation-min.png
  [28]: https://www.learncpp.com/blog/wp-content/uploads/images/CppTutorial/cppreference/if-init-min.png
  [29]: https://isocpp.org/std/the-standard
  [30]: https://github.com/cplusplus/draft/tree/master/papers
  [31]: javascript:void(0)
  [32]: https://www.learncpp.com/cpp-tutorial/introduction-to-object-oriented-programming/
  [33]: https://www.learncpp.com/
  [34]: https://www.learncpp.com/cpp-tutorial/chapter-13-summary-and-quiz/
