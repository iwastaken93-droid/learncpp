title: "1.6 — Uninitialized variables and undefined behavior – Learn C++"
...
# 1.6 â€" Uninitialized variables and undefined behavior

[*Alex*][1] February 1, 2019, 9:56 am PST November 29, 2024
...
In lesson [1.4 -- Variable assignment and initialization][2], we noted that when no initializer is provided, the variable is default-initialized. In most cases (such as this one), default-initialization performs no actual initialization. Thus we'd say `x` is uninitialized. We're focused on the outcome (the object has not been given a known value), not the process.
...
Some compilers, such as Visual Studio, *will* initialize the contents of memory to some preset value when you're using a debug build configuration. This will not happen when using a release build configuration. Therefore, if you want to run the above program yourself, make sure you're using a *release build configuration* (see lesson [0.9 -- Configuring your compiler: Build configurations][3] for a reminder on how to do that).
...
We discuss `sizeof()` in lesson [4.3 -- Object sizes and the sizeof operator][5].
...
We show examples of unspecified behavior in lesson [6.1 -- Operator precedence and associativity][6].
