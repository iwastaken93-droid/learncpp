title: "1.5 — Introduction to iostream: cout, cin, and endl – Learn C++"
...
[*Alex*][1] January 12, 2015, 4:53 pm PST February 5, 2025
...
We discuss what the *std::* prefix actually does in lesson [2.9 -- Naming collisions and an introduction to namespaces][2].
...
Even though '\n' is represented in source code as two symbols, it is treated by the compiler as a single linefeed (LF) character (with ASCII value 10), and thus is conventionally single quoted (unless embedded into existing double-quoted text). We discuss this more in lesson [4.11 -- Chars][3].

When '\n' is output, the library doing the outputting is responsible for translating this single LF character into the appropriate newline sequence for the given OS. See [Wikipedia][4] for more information on OS conventions for newlines.
...
2.  More importantly, it helps avoid inadvertent multicharacter literals. We cover multicharacter literals and some of the unexpected output they can cause in lesson [4.11 -- Chars][3].

Single quotes should be preferred in non-output cases.

We'll cover what '\n' is in more detail when we get to the lesson on chars ([4.11 -- Chars][3]).
...
- If your screen closes immediately after entering a number, please see lesson [0.8 -- A few common C++ problems][5] for a solution.
- If you are using CLion and "You entered� has a space before it, this is a bug in CLion. See [the CLion bug tracker][6] for workarounds.
...
The C++ I/O library does not provide a way to accept keyboard input without the user having to press *enter*. If this is something you desire, you'll have to use a third party library. For console applications, we'd recommend [pdcurses][7], [FXTUI][8], [cpp-terminal][9], or [notcurses][10]. Many graphical user interface libraries have their own functions to do this kind of thing.
...
We discuss how to detect and handle extraction failures, handle extraneous input, and clear `std::cin` in lesson [9.5 -- std::cin and handling invalid input][11].
...
We'll talk more about operators in lesson [1.9 -- Introduction to literals and operators][12].

As an aside…

If you're wondering why C++ uses `std::cout` and `std::cin` instead of something else, see <https://en.wikipedia.org/wiki/Standard_streams>.
...
1.4Variable assignment and initialization
  ...
  [4]: https://en.wikipedia.org/wiki/Newline
  [5]: https://www.learncpp.com/cpp-tutorial/a-few-common-cpp-problems/
  [6]: https://youtrack.jetbrains.com/issue/CPP-17579
  [7]: https://pdcurses.org/
  [8]: https://github.com/ArthurSonzogni/FTXUI
  [9]: https://github.com/jupyter-xeus/cpp-terminal
  [10]: https://github.com/dankamongmen/notcurses
  [11]: https://www.learncpp.com/cpp-tutorial/stdcin-and-handling-invalid-input/
  [12]: https://www.learncpp.com/cpp-tutorial/introduction-to-literals-and-operators/
  [13]: javascript:void(0)
  [14]: https://www.learncpp.com/cpp-tutorial/signed-integers/
  [15]: https://www.learncpp.com/cpp-tutorial/uninitialized-variables-and-undefined-behavior/
