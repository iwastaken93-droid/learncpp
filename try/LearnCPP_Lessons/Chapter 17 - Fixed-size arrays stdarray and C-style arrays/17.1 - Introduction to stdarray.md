title: "17.1 — Introduction to std::array – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/introduction-to-stdarray/
...
[*Alex*][1] September 14, 2015, 4:39 pm PDT July 22, 2024

In lesson [16.1 -- Introduction to containers and arrays][2], we introduced containers and arrays. To summarize:
...
In lesson [16.10 -- std::vector resizing and capacity][3], we mentioned that arrays fall into two categories:
...
We cover non-type template parameters in lesson [11.9 -- Non-type template parameters][4].
...
We covered aggregate initialization for structs in lesson [13.8 -- Struct aggregate initialization][5].
...
If more initializers are provided in an initializer list than the defined array length, the compiler will error. If fewer initializers are provided in an initializer list than the defined array length, the remaining elements without initializers are value initialized:
    ...
    int main()
    {
        ...
        std::array<int, 4> b { 1, 2 };          // b[8] and b[9] are value initialized
        return 0;
    }
...
Just like a `std::vector`, the most common way to access elements of a `std::array` is by using the subscript operator (`operator[]`):
    ...
    {
        constexpr std::array<int, 5> prime{ 2, 3, 5, 7, 11 };
        std::cout << prime[9]; // print the value of element with index 3 (7)
...
17.2std::array length and indexing
...
16.xChapter 16 summary and quiz
  ...
  [3]: https://www.learncpp.com/cpp-tutorial/stdvector-resizing-and-capacity/
  [4]: https://www.learncpp.com/cpp-tutorial/non-type-template-parameters/
  [5]: https://www.learncpp.com/cpp-tutorial/struct-aggregate-initialization/
  [6]: javascript:void(0)
  [7]: https://www.learncpp.com/cpp-tutorial/stdarray-length-and-indexing/
  [8]: https://www.learncpp.com/
  [9]: https://www.learncpp.com/cpp-tutorial/chapter-16-summary-and-quiz/
