title: "17.2 — std::array length and indexing – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/stdarray-length-and-indexing/
...
# 17.2 â€" std::array length and indexing

[*Alex*][1] September 11, 2023, 3:32 pm PDT August 15, 2024

In lesson

16.3 -- std::vector and the unsigned length and subscript problem

, we discussed the unfortunate decision to make the standard library container classes use unsigned values for lengths and indices. Because `std::array` is a standard library container class, it is subject to the same issues.
...
We cover class templates (which includes struct templates) in lesson [13.13 -- Class templates][2] and non-type template parameters in lesson [11.9 -- Non-type template parameters][3].
...
Visual Studio incorrectly triggers warning C4365 for the above example. The issue has been [reported to Microsoft][4].
...
This defect has been addressed in C++23 by [P2280][5]. At the time of writing, few compilers currently [support][6] this feature.

A workaround is to make `foo()` a function template where the array length is a non-type template parameter. This non-type template parameter can then be used inside the function. We discuss this further in lesson [17.3 -- Passing and returning std::array][7].
...
In the prior lesson [17.1 -- Introduction to std::array][8], we covered that the most common way to index a `std::array` is to use the subscript operator (`operator[]`). No bounds checking is done in this case, and passing in an invalid index will result in undefined behavior.
...
However, if either of these functions are called with a non-constexpr signed integral value, the conversion to `std::size_t` is considered narrowing and your compiler may emit a warning. We discuss this case further (using `std::vector`) in lesson
...
[Show Solution][9]
    ...
    int main()
    {
        ...
        std::cout << arr[10] << arr.at(1) << std::get<1>(arr) << '\n';
        return 0;
    }

Next lesson

17.3Passing and returning std::array

Back to table of contents

Previous lesson

17.1Introduction to std::array

  [1]: https://www.learncpp.com/author/Alex/
  [2]: https://www.learncpp.com/cpp-tutorial/class-templates/
  [3]: https://www.learncpp.com/cpp-tutorial/non-type-template-parameters/
  [4]: https://developercommunity.visualstudio.com/t/Bug:-C4365-triggers-incorrectly-for-cons/10372737
  [5]: https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2022/p2280r4.html
  [6]: https://en.cppreference.com/w/cpp/compiler_support/23
  [7]: https://www.learncpp.com/cpp-tutorial/passing-and-returning-stdarray/
