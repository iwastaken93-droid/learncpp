title: "17.7 — Introduction to C-style arrays – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/introduction-to-c-style-arrays/
...
# 17.7 â€" Introduction to C-style arrays

[*Alex*][1] June 27, 2007, 5:20 pm PDT October 13, 2024
...
Variable-length arrays are not valid C++, and should not be used in C++ programs. If your compiler allows these arrays, you probably forgot to disable compiler extensions (see [0.10 -- Configuring your compiler: Compiler extensions][3]).
...
Just like with a `std::array`, C-style arrays can be indexed using the subscript operator (`operator[]`):
    ...
    int main()
    {
        ...
        arr[7] = 7; // use subscript operator to index array element 1
        std::cout << arr[7]; // prints 7
        return 0;
    }
...
If more initializers are provided in an initializer list than the defined array length, the compiler will error. If fewer initializers are provided in an initializer list than the defined array length, the remaining elements without initializers are value initialized:
    ...
    int main()
    {
        ...
        int b[4] { 1, 2 };          // arr[8] and arr[3] are value initialized
        return 0;
    }
...
You can see the full list of headers that define these functions in the [cppreference documentation for the size functions][4].
...
We cover array decay in the next lesson [17.8 -- C-style array decay][5].
...
17.6std::array and enumerations
  ...
  [4]: https://en.cppreference.com/w/cpp/iterator/size
  [5]: https://www.learncpp.com/cpp-tutorial/c-style-array-decay/
  [6]: javascript:void(0)
  [7]: https://www.learncpp.com/
  [8]: https://www.learncpp.com/cpp-tutorial/stdarray-and-enumerations/
