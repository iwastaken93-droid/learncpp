title: "17.9 — Pointer arithmetic and subscripting – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/pointer-arithmetic-and-subscripting/
...
# 17.9 â€" Pointer arithmetic and subscripting

[*Alex*][1] August 15, 2015, 4:31 pm PDT August 10, 2024

In lesson [16.1 -- Introduction to containers and arrays][2], we mentioned that arrays are stored sequentially in memory. In this lesson, we'll take a deeper look at how arrays indexing math works.
...
In the prior lesson ([17.8 -- C-style array decay][3]), we noted that `operator[]` can be applied to a pointer:
...
Remember, given some pointer `ptr`, both `*(ptr + 1)` and `ptr[7]` return the *next object* in memory (based on the type being pointed to). Next is a relative term, not an absolute one. Thus, if `ptr` is pointing to element 0, then both `*(ptr + 1)` and `ptr[7]` will return element 1. But if `ptr` is pointing to element 3 instead, then both `*(ptr + 1)` and `ptr[7]` will return element 4!
...
The following example demonstrates this:
    ...
    int main()
    {
        ...
        std::cout << *ptr << ptr[0] << '\n'; // prints 99
        // Prove that ptr[7] is element 1
        std::cout << *(ptr+1) << ptr[7] << '\n'; // prints 88
        // Now set ptr to point at element 3
        ptr = &arr[3];
        // Prove that we're pointing at element 3
        std::cout << *ptr << ptr[0] << '\n'; // prints 66
        // Prove that ptr[7] is element 4!
        std::cout << *(ptr+1) << ptr[7] << '\n'; // prints 55
     
        return 0;
    }

However, you'll also note that our program is a lot more confusing if we can't assume that `ptr[7]` is always the element with index 1. For this reason, we recommend using subscripting only when indexing from the start of the array (element 0). Use pointer arithmetic only when doing relative positioning.
...
We just covered that `*(ptr+1)` returns the *next object* in memory. And `ptr[7]` is just a convenient syntax to do the same.
...
If you look at the [documentation][4] for range-based for loops, you'll see that they are typically implemented something like this:
...
We have more quiz questions on pointer arithmetic in the next lesson ([17.10 -- C-style strings][6]).

Next lesson

17.10C-style strings

Back to table of contents

Previous lesson

17.8C-style array decay

  [1]: https://www.learncpp.com/author/Alex/
  [2]: https://www.learncpp.com/cpp-tutorial/introduction-to-containers-and-arrays/
  [3]: https://www.learncpp.com/cpp-tutorial/c-style-array-decay/
  [4]: https://en.cppreference.com/w/cpp/language/range-for
  [5]: javascript:void(0)
  [6]: https://www.learncpp.com/cpp-tutorial/c-style-strings/
