title: "17.3 — Passing and returning std::array – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/passing-and-returning-stdarray/
...
# 17.3 â€" Passing and returning std::array

[*Alex*][1] September 11, 2023, 3:46 pm PDT December 2, 2024
...
We cover function templates in lesson [11.6 -- Function templates][2].
...
We cover preconditions in lesson [9.6 -- Assert and static_assert][3].
...
In the example above, you may be wondering why we use `static_assert (N > 3);` instead of `static_assert (std::size(arr) > 3)`. The latter won't compile prior to C++23 due to the language defect mentioned in the prior lesson ([17.2 -- std::array length and indexing][4]).
...
In such cases, a copy of the `std::array` will be made, but if all of the above are true, the performance hit will be minor, and sticking with the most conventional way to return data to the caller may be the best choice.
    ...
    int main()
    {
        ...
        std::cout << "The value of element 2 is " << arr[7] << '\n';
        return 0;
    }
...
17.2std::array length and indexing
  ...
  [4]: https://www.learncpp.com/cpp-tutorial/stdarray-length-and-indexing/
  [5]: javascript:void(0)
  [6]: https://www.learncpp.com/cpp-tutorial/stdarray-of-class-types-and-brace-elision/
