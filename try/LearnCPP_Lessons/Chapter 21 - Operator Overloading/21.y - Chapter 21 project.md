title: "21.y — Chapter 21 project – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/chapter-21-project/
...
# 21.y â€" Chapter 21 project

[*Alex*][1] March 30, 2023, 1:02 pm PDT September 13, 2024

A tip o' the hat to reader Avtem for conceiving of and collaborating on this project.

Project time

Let's implement the classic game [15 Puzzle][2]!

In 15 Puzzle, you begin with a randomized 4Ã---4 grid of tiles. 15 of the tiles have numbers 1 through 15. One tile is missing.
...
You can play a few rounds [on this site][3]. It will help you to understand how this game works and how it should be implemented.
...
|:-----------------------------|:--------------------------------|:--------|
| class Board                  | Display the game board <br/>… | …     |
...
- `class Board`: Our game is a 4Ã---4 grid of tiles. The main purpose of this class is to store and manage the 2-dimensional array of tiles. This class is also responsible for randomizing, moving tiles, displaying the board, and checking whether our board is solved.
...
Our game board is a 4Ã---4 grid of tiles that can slide around. Therefore, it will be useful to have a `Tile` class that represents one of the numbered tiles on our 4Ã---4 grid or the missing tile. Each tile should be able to:
...
Goal: Create a solved board (4Ã---4 grid of tiles) and display it on the screen.

Define a `Board` class that will represent 4Ã---4 grid of tiles. A newly created `Board` object should be in the solved state. To display the board, first print `g_consoleLines` (defined in code snippet below) empty lines and then print the board itself. Doing so will ensure that any prior output is pushed out of view so that only the current board is visible on the console.
...
- A static function that returns a Direction with a randomized `Type`. You can use the `Random::get()` function from the ["Random.h�][5] header to generate a random number.
...
22.1Introduction to smart pointers and move semantics
...
21.xChapter 21 summary and quiz
  ...
  [2]: https://en.wikipedia.org/wiki/15_puzzle
  [3]: https://15puzzle.netlify.app/
  [4]: javascript:void(0)
  [5]: https://www.learncpp.com/cpp-tutorial/global-random-numbers-random-h/
  [6]: https://www.learncpp.com/
  [7]: https://www.learncpp.com/cpp-tutorial/chapter-21-summary-and-quiz/
