title: "21.9 — Overloading the subscript operator – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/overloading-the-subscript-operator/
date: 2026-04-01 03:29
tags: 
description: 
...
# 21.9 â€" Overloading the subscript operator

[*Alex*][1] October 19, 2007, 9:50 am PDT February 2, 2025
...
This is both easy syntactically and from a comprehension standpoint. When `list[7]` evaluates, the compiler first checks to see if there's an overloaded operator\[\] function. If so, it passes the value inside the hard braces (in this case, 2) as an argument to the function.
...
Let's take a closer look at how `list[7] = 3` evaluates. Because the subscript operator has a higher precedence than the assignment operator, `list[7]` evaluates first. `list[7]` calls operator\[\], which we've defined to return a reference to `list.m_list[7]`. Because operator\[\] is returning a reference, it returns the actual `list.m_list[7]` array element. Our partially evaluated expression becomes `list.m_list[7] = 3`, which is a straightforward integer assignment.

In lesson [12.2 -- Value categories (lvalues and rvalues)][2], you learned that any value on the left hand side of an assignment statement must be an l-value (which is a variable that has an actual memory address). Because the result of operator\[\] can be used on the left hand side of an assignment (e.g. `list[7] = 3`), the return value of operator\[\] must be an l-value. As it turns out, references are always l-values, because you can only take a reference of variables that have memory addresses. So by returning a reference, the compiler is satisfied that we are returning an l-value.

Consider what would happen if operator\[\] returned an integer by value instead of by reference. `list[7]` would call operator\[\], which would return the *value of* list.m_list\[2\]. For example, if m_list\[2\] had the value of 6, operator\[\] would return the value 6. `list[7] = 3` would partially evaluate to `6 = 3`, which makes no sense! If you try to do this, the C++ compiler will complain:
...
The good news is that we can define a non-const and a const version of operator\[\] separately. The non-const version will be used with non-const objects, and the const version with const-objects.
    ...
    {
        IntList list{};
        list[7] = 3; // okay: calls non-const version of operator[]
        std::cout << list[7] << '\n';
        const IntList clist{};
        // clist[7] = 3; // compile error: clist[7] returns const reference, which we can't assign to
        std::cout << clist[7] << '\n';
        return 0;
    }
...
In the above example, note that the implementations of `int& IntList::operato` and `const int& IntList::operato const` are identical. The only difference is the return type of the function.
...
Consider the following example:
    ...
    int main()
    {
        ...
        list [7] = 3; // error: this will assume we're accessing index 2 of an array of IntLists
        delete list;
        return 0;
    }
...
The proper syntax would be to dereference the pointer first (making sure to use parenthesis since operator\[\] has higher precedence than operator\*), then call operator\[\]:
    ...
    {
        IntList* list{ new IntList{} };
        (*list)[7] = 3; // get our IntList object, then call overloaded operator[]
        delete list;
        return 0;
    }
...
See [20.6 -- Introduction to lambdas (anonymous functions)][4] for more information on lambdas.

Tip

Since maps are common, the standard library offers [`std::map`][5], which is not currently covered on learncpp. Using `std::map`, we can simplify our code to
...
21.10Overloading the parenthesis operator

Back to table of contents

Previous lesson

21.8Overloading the increment and decrement operators

  [1]: https://www.learncpp.com/author/Alex/
  [2]: https://www.learncpp.com/cpp-tutorial/value-categories-lvalues-and-rvalues/
  [3]: javascript:void(0)
  [4]: https://www.learncpp.com/cpp-tutorial/introduction-to-lambdas-anonymous-functions/
  [5]: https://en.cppreference.com/w/cpp/container/map
  [6]: https://www.learncpp.com/cpp-tutorial/overloading-the-parenthesis-operator/
