title: "18.2 — Introduction to iterators – Learn C++"
...
[*Alex*][1] December 17, 2019, 10:38 am PST February 11, 2025
...
The examples in this lesson use a C++17 feature called `class template argument deduction` to deduce the template arguments for a template variable from its initializer. In the example above, when the compiler sees `std::array arr{ 0, 1, 2, 3, 4, 5, 6 };`, it will deduce that we want `std::array<int, 7> arr { 0, 1, 2, 3, 4, 5, 6 };`.

If your compiler is not C++17 enabled, you'll get an error that says something like, "missing template arguments before 'arr'�. In that case, your best bet is to enable C++17, as per lesson

0.12 -- Configuring your compiler: Choosing a language standard

. If you can not, you can replace the lines that use class template argument deduction with lines that have explicit template arguments (e.g. replace `std::array arr{ 0, 1, 2, 3, 4, 5, 6 };` with `std::array<int, 7> arr { 0, 1, 2, 3, 4, 5, 6 };`
...
In lesson [8.10 -- For statements][2], we noted that using `operator<` was preferred over `operator!=` when doing numeric comparisons in the loop condition:
...
"Iterator invalidation� section of `std::vector` on cppreference
...
18.3Introduction to standard library algorithms
...
(h/t to nascardriver for significant contributions to this lesson)
  ...
  [3]: https://www.learncpp.com/cpp-tutorial/introduction-to-standard-library-algorithms/
  [4]: https://www.learncpp.com/
  [5]: https://www.learncpp.com/cpp-tutorial/sorting-an-array-using-selection-sort/
