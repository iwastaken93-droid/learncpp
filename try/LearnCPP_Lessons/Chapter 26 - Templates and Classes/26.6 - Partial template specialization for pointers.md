title: "26.6 — Partial template specialization for pointers – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/partial-template-specialization-for-pointers/
...
# 26.6 â€" Partial template specialization for pointers

[*Alex*][1] December 5, 2016, 11:15 pm PST March 29, 2024

In previous lesson [26.4 -- Class template specialization][2], we took a look at a simple templated `Storage` class, along with a specialization for type `double`:
...
Unfortunately, this doesn't work for a simple reason: as of the time of writing (C++23) functions cannot be partially specialized. As we noted in lesson [26.5 -- Partial template specialization][3], only classes can be partially specialized.
...
1.  Have `Storage<T*>` make a copy of the object on the heap. If you do all the heap memory management yourself, this requires overloading the constructor, copy constructor, copy assignment, and destructor. An easier alternative is to just use `std::unique_ptr` (which we cover in lesson [22.5 -- std::unique_ptr][4]):
...
26.xChapter 26 summary and quiz
...
26.5Partial template specialization
  ...
  [2]: https://www.learncpp.com/cpp-tutorial/class-template-specialization/
  [3]: https://www.learncpp.com/cpp-tutorial/partial-template-specialization/
  [4]: https://www.learncpp.com/cpp-tutorial/stdunique_ptr/
  [5]: https://www.learncpp.com/cpp-tutorial/chapter-26-summary-and-quiz/
