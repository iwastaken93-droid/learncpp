title: "15.1 — The hidden "this" pointer and member function chaining – Learn C++"
source: https://www.learncpp.com/cpp-tutorial/the-hidden-this-pointer-and-member-function-chaining/
...
# 15.1 â€" The hidden "thisâ€? pointer and member function chaining

[*Alex*][1] September 6, 2007, 10:20 am PDT December 29, 2024
...
We cover `operator->` in lesson [13.12 -- Member selection with pointers and references][2].
...
In this context, the `static` keyword means the function is not associated with objects of the class, but instead is treated as if it were a normal function inside the scope region of the class. We cover static member functions in lesson [15.7 -- Static member functions][3].
...
As noted in previous lessons ([14.12 -- Delegating constructors][4]), constructors are only for initialization of new objects, and should not be called directly. Doing so will result in unexpected behavior.
